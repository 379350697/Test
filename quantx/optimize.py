...
def grid_search(candles, strategy_name: str, param_grid: dict[str, list], base_config: BacktestConfig):
    ...
    shared_data_hash = stable_hash([(c.ts.isoformat(), c.open, c.high, c.low, c.close, c.volume) for c in candles])
    out = []
    for params in combos:
        res = run_backtest(candles, strategy_name, params, base_config, use_indicator_cache=True, data_hash=shared_data_hash)
...
def walk_forward(candles, strategy_name: str, params: dict, base_config: BacktestConfig, splits: int = 3, train_ratio: float = 0.7):
    ...
    for i in range(splits):
        ...
        train = seg[:cut]
        test = seg[cut:]
        train_res = run_backtest(train, strategy_name, params, base_config, use_indicator_cache=True)
        test_res = run_backtest(test, strategy_name, params, base_config, use_indicator_cache=True)
        windows.append(
            {
                "window": i,
